...
public class RFX1_TransformMotion : MonoBehaviour
{
    ...
    public void Initialize(Vector3 targetPostion, bool useFixedDir)
    {
        targetDirection = (targetPostion - transform.position).normalized;
        useFixedDirection = useFixedDir;
    }
    ...
    void UpdateWorldPosition()
    {
        ...
        if (RandomMoveRadius > 0)
        {
            ...
            if (Target != null && !useFixedDirection)
            {
        ...
        if (!isCollided && !isOutDistance)
        {
            ...
            {
                var currentForwardVector = (targetDirection + randomOffset) * Speed * Time.deltaTime;
    ...
    private void OnDestroy()
    {
        ...
        {
            EffectDestroyer temp = GetComponent<EffectDestroyer>();
            if (temp != null)
            {
                temp.DestroyEffect();
            }
